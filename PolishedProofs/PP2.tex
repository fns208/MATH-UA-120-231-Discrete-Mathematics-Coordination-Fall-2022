...
Please choose \textbf{one} of the following two exercises from the textbook, or the exercise that is typed. Begin with ``Claim:" and write the statement you intend to prove. Then write ``Proof:" and the proof. You can choose your own end-of-proof marker for flair. 
$$12.27:\text{Prove that for all sets } A, B, C,  \text{ we have } A \;\Delta \; (B \Delta C) = (A \Delta B) \; \Delta\; C $$ 
$$ \text{ \textbf{or} Prove that for all sets } A, B, C,  \text{ we have } A \cap (B \Delta C) = (A \cap B) \; \Delta \; (A \cap C) $$
